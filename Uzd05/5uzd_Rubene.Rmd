...
Process aizņema aptuveni 30 sekundes.
...
leaflet() %>%
  ...
  addLayersControl(
    ...
    options = layersControlOptions(collapsed = FALSE))
```
Man bija aizdomas, ka kādā no malām poligoni neaizsniegsies līdz pašām lapas malām, attiecīgi radot pārrāvumus starp atsevišķiem rastriem tomēr izskatās, ka tas tā nav noticis
...
leaflet() %>%
  ...
  addLayersControl(
    ...
    options = layersControlOptions(collapsed = FALSE))
```
Pārrāvumu nav, tomēr redzams, ka tieši zem lapu robežām vērtības ir zemākas - tas tāpēc, ka nav iekļauti uz robežām vai tuvu tām ārpusē esošie poligoni, kas tad attiecīgi nav pierēķināti priežu mežu īpatsvaram un rada kļūdas datos.
...
Izmantošu pieeju ar st_join, jo pagaidām tā šķita precīzākā. Te uzreiz varu mērīt laiku, jo pa vidu neko neapskatīšu - funkciju darbība ir skaidra. Vēr'tibas salīdzināšu pašās beigās visiem sešiem apakšuzdevumiem kopā.

```{r, warning = FALSE, message = FALSE}
SakumsUzd4 <- Sys.time()

uzd4_mezi <- st_join(mezi, lapas, left = FALSE)
...
Ieviešu un pielietoju funkciju (šoreiz jānomaina bija tikai izvades rastra filename:
...
uzd5_priedes <- crop(rast("5uzd_priedes100m.tif"), mezi)

BeigasUzd5 <- Sys.time()
print(BeigasUzd5 - SakumsUzd5)
```

Process aizņēma aptuveni pusotru minūti

```{r}
rm(basename, BeigasUzd5 ,i ,nosaukums, SakumsUzd5)
```

xxxxx vēl jāsalīdzina rezultāti!!
...
Mans dators nav jaudīgs, tomēr tāpat vismaz pamēģināšu?
