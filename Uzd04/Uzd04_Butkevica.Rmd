...
Iepriekšejos uzdevumos neesmu saglabājusi apvienoto Centra virsmežņiecības failu, tapēc izdarīšu to tagad pasleptājos komandu blokos.

```{r, saglabat apvienoto Centra parquet, include=FALSE, results='hide', warning = FALSE, message = FALSE}
...
rm(parquet_dir_cels, parquet_visi)

library(sf)
# Pārliecinieties, ka ģeometrijas ir MULTIPOLYGON
apvienotais_parquet <- apvienotais_parquet %>%
  mutate(geometry = st_cast(geometry, "MULTIPOLYGON"))

# Pārbaudām nederīgās ģeometrijas
...
# Ja ir, tad labojam nederīgās ģeometrijas
...
##################nodzes mainīgus, kas vairs nav nepieciešami
...
# Saglabā Parquet failu
dir_visi_mezi <- "C:/Users/user/Desktop/HiQBioDiv_macibas/4uzd/"
dir.create(dir_visi_mezi, recursive = TRUE)

st_write_parquet(apvienotais_parquet, "C:/Users/user/Desktop/HiQBioDiv_macibas/4uzd/Centra_visi.parquet")
```


# Pirmais uzdevums

Sagatavojiet funkciju ar nosaukumu mana_funkcija, kura (secīgi):
...
**Atbilde:** Funkcijas izpilde aizņēma 528,49 sekundes jeb 8,8 minūtes, tika izmantoti 21,09 GB RAM, un R sesijā pastāvīgi bija nodarbināti 3 CPU, periodiski mazāk, bet vairāk laikam nebija. Papildus līdz 3 CPU tika izmantoti uz System un Memory Compression.
...
Sagatavojiet katras nodaļas MVR datus atsevišķā geoparquet failā, kura nosaukums satur nodaļas nosaukumu. Iteratīva procesa, piemēram, for cikla, veidā, ieviesiet sagatavoto funkciju katrai nodaļai atsevišķi. Cik daudz laika aizņem šis uzdevums? Cik CPU kodolus tas aizņem periodiski, cik - patstāvīgi? Vai pietika oeratīvās atmiņas uzdevuma veikšanai, kādu tās apjomu R izmantoja?

```{r, Trešais uzdevums, echo=TRUE, results='hide', warning = FALSE, message = FALSE}
...
**Atbilde:** Funkcijas izpilde aizņēma 2226,89 sekundes jeb 37,11 minūtes, tika izmantoti 24,8 GB RAM, un R sesija pastāvīgi izmantoja 3 CPU kodolus, periodiski mazāk. Līdz 7 CPU aizņema System un Memory Compression.
...
Atkārtojiet iepriekšējo uzdevumu, izmantojot {doParallel} un {foreach}, bet nosakiet klāseteri kā tieši vienu CPU kodolu lielu. Cik daudz laika aizņem šis uzdevums? Cik CPU kodolus tas aizņem periodiski, cik - patstāvīgi? Vai pietika oeratīvās atmiņas uzdevuma veikšanai, kādu tās apjomu R izmantoja?


```{r, Ceturtais uzdevums, echo=TRUE, results='hide', warning = FALSE, message = FALSE}
library(doParallel)
library(foreach)
...
**Atbilde:** Neredzu nekādas īpašas atšķirības no iepriekšējā varianta. Vienīgais, ka ar R sesiju iedarbinātie CPU nemainās un vienmēr ir stabils skaits – 3. Izmantotas 24.2 GB no RAM. Patērētais laiks: 1670.99 sekundes jeb 27.85 minūtes.
...
Atkārtojiet iepriekšējo uzdevumu paralēli uz vismaz diviem CPU kodoliem. Cik daudz laika aizņem šis uzdevums? Cik CPU kodolus tas aizņem periodiski, cik - patstāvīgi? Vai pietika oeratīvās atmiņas uzdevuma veikšanai, kādu tās apjomu R izmantoja?


Izmeģināju ar četriem, bet saņēu kļūdu: Error in { : task 1 failed - "cannot allocate vector of size 10.0 Gb".

Veicu izmēģinājumu ar diviem:


```{r, Piektais uzdevums, echo=TRUE, results='hide', warning = FALSE, message = FALSE}
library(doParallel)
library(foreach)
...
**Atbilde:** Pastāvīgi tika izmantoti 6 CPU: trīs un trīs paralēli, kā arī periodiski vēl 1–3. Izmantotas 25,1 GB RAM. Patērētais laiks: 1077,97 sekundes jeb 17,97 minūtes.
