...
packages <- c("sf", "terra", "raster", "arrow", "dplyr", "exactextractr", "doParallel", "sfheaders", "microbenchmark", "sfarrow", "ggplot2")
...
rm(packages)
```
<br><br> 1) Izmantojiet {exactextractr}, lai 500 m buferzonās ap projekta Zenodo repozitorijā pieejamo 100 m šūnu centriem (pts100_sauszeme.parquet), kuri atrodas piektajā uzdevumā izvēlētajās kartes lapās, aprēķinātu sestā uzdevuma ceturtajā apakšpunktā izveidotā rastra katras klases platības īpatsvaru.
<br> 1.1.) 5.uzd karšu lapas nebiju saglabājis, tās uztaisu pa jaunam.
```{r 1. uzdevums - karšu lapu sagatavošana, warning=FALSE}
...
```
<br> 1.2.) Aprēķināšu katras vecuma un mežu tipa klases īpatsvaru 500 metru buferzonā ap references centroīdiem.
```{r 1. uzdevums - sagatavošanās exact_extract izpildei, warning=FALSE}
...
```{r 1. (1) uzdevums - paralēlā izpilde, warning=FALSE}
...
```
<br> Brīvi izvēlaties desmit blakus esošus 1km kvadrātus, kas atrodas trešajā uzdevumā ar Lauku atbalsta dienesta datiem aptvertajā teritorijā. Izmantojiet projekta Zenodo repozitorijā dotos 300 m un 100 m tīklu centrus, kas atrodas izvēlētajos kvadrātos. Aprēķiniet 3 km buferzonas ap ik centru
```{r 2. uzdevums - sagatavošanās un buferzonas, warning=FALSE}
...
```{r 2. uzdevums - funkciju definēšana, warning=FALSE}
...
```
<br> `microbenchmark` man met nesaprotamas kļūdas ārā, ja norādu vairāk nekā 1 atkārtošanas reizes(ar 1 viss izpildās bez problēmām). Mēģināju visādi risināt, nekādīgi nesanāca. Intereses pēc saviem spēkiem uzrakstīšu laika pārbaudes komandrindas 10 atkārtojumiem katrai funkcijai, izmantojot stabilu Sys.time().
```{r Laika pārbaude, results='hide', echo=TRUE, warning=FALSE}
...
```
<br> Visātrāk bija rēķināt 300 metru punktu buferiem un 100 metru rastra izšķirtspējā. 300 metros punktiem un 10 metru rastram rezultāts arī ir diezgan ātri, bet problēma ka ir daudzas NA vērtības.  
<br> Maksimālā starpība visos gadījumos sanāk ir max 1%, neatkarīgi no punktu attāluma un rastru izšķirtspējas. Lai nebūtu NA tad rēķina 100 metru punktiem un 100 metru rastram (protams ja mērķis ir raksturot īpatsvaru 100 metru vektorā).
<br> Tendences īsti neredzu - ir poligoni kuros 300 metru aprēķini rāda mazākas vērtības, ir, kur lielākas, ir arī kur sakrīt... 